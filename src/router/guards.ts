...
// 判断错误类型的辅助函数
function determineErrorType(error: any): 'network' | 'permission' | 'other' {
      ...
      if (routeStore.isGenerate) {
        ...
        try {
          ...
          if (errorType === 'network') {
            ...
            // 其他错误（如权限不足）则登出
            globalErrorHandler.handleApiError(error, { context: '权限获取' });
            userStore.logout();
          }
        }
        // 动态路由生成并注册后，重新进入当前路由
    ...
    if (to.fullPath !== from.fullPath) {
      if (to.meta.cache) {
        ...
        if (componentName) {
          ...
          console.warn('[super-admin] 该页面组件未设置组件名，会导致缓存失效，请检查')
        }
      }
    }
  })
